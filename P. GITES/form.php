...
<body class="formulaire">
    ...
    <form action="form.php" method="POST" enctype="multipart/form-data">
        <div class="box">
            <div>
                <input type="hidden" name="hidden" required value="<?php if (isset($_GET['id'])) {echo $lodge->getIdlodge();} ?>">

                <label for="name">Nom :</label>
                <input type="text" name="name" id="name" required value="<?php if (isset($_GET['id'])) {echo $lodge->getLodgename();} ?>">

                <label for="price">Prix :</label>
                <input required type="number" name="price" step=".01" id="price" value="<?php if (isset($_GET['id'])) {echo $lodge->getPrice();} ?>">
            </div>

            <div>
                <label for="location">Localisation :</label>
                <input required type="text" name="location" id="location" value="<?php if (isset($_GET['id'])) {echo $lodge->getLocation();} ?>">
            </div>

            <div>
                <label for="bedroom">Nombre de chambre :</label>
                <input required type="number" name="bedroom" id="bedroom" min="0" max="99" step="1" value="<?php if (isset($_GET['id'])) {echo $lodge->getBedroom();} ?>">

                <label for="bathroom">Nombre de salle de bain :</label>
                <input required type="number" name="bathroom" id="bathroom" min="0" max="99" step="1" value="<?php if (isset($_GET['id'])) {echo $lodge->getBathroom();} ?>">
            </div>
            <div>
            ...
            <div class="text_area">
                <textarea required name="description" id="description"><?php if (isset($_GET['id'])) {echo $lodge->getDescription();} ?></textarea>
            </div>
