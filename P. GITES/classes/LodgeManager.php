...
class LodgeManager
{
    ...
    {
        require "classes/class.booking.php";
        $lodge = [];
        $req = $this->_db->prepare('SELECT * FROM `lodge` NATURAL JOIN `booking` WHERE `location` = :location AND (:arrival NOT BETWEEN `arrival` AND `departure`) AND (:departure NOT BETWEEN `arrival` AND `departure`');
